...
func (conversion *PageConversion) ConvertImageMap(context *ditaconvert.Context, dec *xml.Decoder, start xml.StartElement) error {
	...
	if ext == "" {
		context.Errors = append(context.Errors, fmt.Errorf("invalid image link %s", href))
		return nil
	}
	if ext == "jpg" {
		ext = "jpeg"
	...
	for _, area := range content.Areas {
		newref := area.Href
		if len(newref) > 0 && newref[0] == '#' {
			newref = path.Base(context.DecodingPath) + newref
		}
		href, _, _, _ := conversion.ResolveLinkInfo(newref)
		_, selector := ditaconvert.SplitLink(href)
		...
		emitStart("a",
			...
			attr("href", href),
			attr("data-focusid", path.Base(selector)),
		)
		emitEnd("a")
	}
